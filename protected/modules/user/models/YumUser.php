...
class YumUser extends YumActiveRecord
{
  ...
  public function relations() {
	...
	public function register($username = null,$password = null,$profile = null) {
		...
		//If you uncomment this line registration does not work.
			...
			$this->profile = $profile;
				...
				foreach(Yum::module('registration')->defaultRoles as $role) 
					Yii::app()->db->createCommand(sprintf(
                        ...
                        //Adding new user role manually. (role_id -> 3).
                        $last_userid=Yii::app()->db->createCommand("SELECT MAX(id) as 'Id' FROM user")->queryAll();
                        $userid = $last_userid[0]['Id'];
                        Yii::app()->db->createCommand('insert into user_role values('.$userid.', 3)')->execute();
